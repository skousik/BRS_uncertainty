...
nominal_omega = 1;
omegabnds = [nominal_omega - 1;nominal_omega + 1];
v = 1; % speed in m/s
...
subs(w{1},[z{1};pA],[-1;0;nominal_omega])
% %% Time to plot
% 
...
shape = -(theta{1}-omegabnds(1))*(theta{1}-omegabnds(2));
...
beta = reshape(beta,size(X))./max(beta);
...
surf(z1,z2,beta)
figure(3)
contour(z1,z2,beta,linspace(0,1,5))
