...
omegabnds = [nominal_omega - 0.5;nominal_omega + 0.5]; % bounds of distribution
v = 0.5; % speed in m/s
...
hXT{1,1} = [0.25^2 - zA'*zA]; % target set is a ball around the origin
...
z3 = linspace(omegabnds(1),omegabnds(2),61); % discretize z3
...
shape = -(theta{1}-omegabnds(1))*(theta{1}-omegabnds(2)); % shape determines distribution of uncertainty
...
beta = (1/length(z3))*crispw_mat*ftheta_vec/double(mu_theta{1}(msspoly(1))); % creates 2601x1 vector of beta values
beta = reshape(beta,size(X)); % creates a grid in XY of betas. scales by max(beta)

figure(2)
surf(z1,z2,beta)
figure(1)
contour(z1,z2,beta,linspace(0.01,1,5))
