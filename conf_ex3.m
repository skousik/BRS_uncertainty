...
% Discretized uncertainty space for plotting w
...
%%
% Create meshgrid representing w
O = ones(gran/2) ;
Z = zeros(gran/2) ;
...
% C = ['r','r','g','g','b','b'] ;
% 
% % Assign a total degree to the polynomial
% dmt = degree_mu_theta_poly ;
%     % This must be even, or else the polynomial will be negative on
%     % some or all of spt(mu_theta)
% 
% for i = 1:2:dmt
%   % Shape of mu_theta
%     % weight of left root
...
%     shape = -1*(t-BthU)^(l_wt)*(t-BthL)^(r_wt) ;
% 
%     mu{1} = boxMoments_shape(th{1}, shape, BthL, BthU) ;
%     scale = 1/double(mu{1}(msspoly(1))) ;
% 
...
%     subplot(313)
%     hold on
%     plot(thS,f_muth*scale,'Color',C(i))
%     grid on
%     xlabel('\Theta')
%     ylabel('\mu_\theta')
% end
